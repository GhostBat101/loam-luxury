...
export function renderHero(): string {
  return `
    <section aria-labelledby="hero-title" class="relative w-full overflow-hidden hairline-border-b min-h-[85vh] lg:min-h-[92vh] flex flex-col justify-between bg-[#071D31]">
      <div aria-hidden="true" class="absolute inset-0 w-full h-full overflow-hidden pointer-events-none">
        <img alt="Panoramic fine art view of Aegean coastline waves breaking on sun-bleached mineral salt rocks" class="w-full h-full object-cover object-center scale-[1.01] hover:scale-[1.03] transition-transform duration-[4000ms] ease-out" src="/images/hero-aegean.jpg" />
        <div class="absolute inset-0 bg-gradient-to-t from-[#071D31] via-[#071D31]/40 to-transparent"></div>
        <div class="absolute inset-0 bg-gradient-to-r from-[#071D31]/75 via-transparent to-[#071D31]/55"></div>
      ...
      <div aria-hidden="true" class="relative z-10 w-full px-6 sm:px-10 lg:px-16 pt-8 flex justify-between items-start select-none">
        <div class="reveal-item flex items-center gap-3 bg-[#071D31]/85 backdrop-blur-md px-4 py-2 border border-[#CFE2FE]/40 text-[#FAFCFF] font-mono text-[9px] tracking-widest uppercase">
          <span class="crosshair w-2.5 h-2.5 text-[#38bdf8]"></span>
          <span class="font-semibold">DATUM // 36°50'35.9"N 25°53'29.8"E</span>
        </div>
        <div class="reveal-item flex items-center gap-3 bg-[#071D31]/85 backdrop-blur-md px-4 py-2 border border-[#CFE2FE]/40 text-[#FAFCFF] font-mono text-[9px] tracking-widest uppercase" style="transition-delay: 150ms;">
          <span class="font-semibold">SALINITY // 38.5 PSU</span>
          <span class="text-[#38bdf8] animate-ping">●</span>
        </div>
      </div>
      <div class="relative z-10 w-full px-6 sm:px-10 lg:px-16 pb-12 pt-28">
